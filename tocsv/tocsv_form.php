...
class tocsvform extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('textarea', 'namelist','NameList',['cols' => 70, 'rows' => 12])
            ->setLabel(
                get_string('lastname') . ' ' . get_string('firstname') . ' ' .
                get_string('middlename') . ',  ' . get_string('email') . get_string('namelist_header','local_tocsv')
            );
        $mform->setType('namelist', PARAM_CLEANHTML);
        $mform->addRule('namelist', get_string('missingnamelist','local_tocsv'), 'required', null, 'client');

        $mform->setDefault(
            'namelist',   get_string('defaultname1','local_tocsv') . PHP_EOL . get_string('defaultname2','local_tocsv')
        );
